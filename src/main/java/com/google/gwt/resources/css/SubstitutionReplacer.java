  ...
  private ListValue substituteDefs(ListValue listValue) {
    ...
    for (Value val : listValue.getValues()) {
      ...
      if (def == null) {
        ...
        if ((!(methodType instanceof JClassType)) || (!dataResourceType.isAssignableFrom((JClassType) methodType) &&
        ...
        } else if (imageResourceType.isAssignableFrom((JClassType) methodType)) {
          expression.append(instance).append(".getURL()");
        }
        expression.append(" + \"')\"");
